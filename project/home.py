...
def endgame():
    if (request.method == "POST"):
        ...
        data = curseur.execute(requete,(str(request.form["Player1Win"]),str(request.form["Player2Win"]),request.form["NbHitPlayer2"],request.form["NbHitPlayer2"],request.form["Time"]))
        connexion.close()
    else:
        connexion = sqlite3.connect("static/game.db")
        curseur = connexion.cursor()
